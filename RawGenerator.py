...
def mneObjectGenerator (data, fs):
    ...
    # Load the .mat file
    path_to_chan = Path(__file__).parent
    Path_to_chan = str((path_to_chan.parent) / 'chanlocs.mat')
    ...
    rotated_coords = np.dot(np.vstack((x_coords, y_coords, z_coords)).T, rotation_matrix.T)
    ...
    montage = mne.channels.make_dig_montage(ch_pos=dict(zip(ch_names, zip(x_coords_rotated, y_coords_rotated, z_coords))),
                                            coord_frame='head')
